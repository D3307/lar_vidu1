    ...
    <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST" class="styled-form">
        ...
        <div class="form-group">
            ...
            <select name="discount_type" id="discount_type" required>
                ...
                <option value="fixed" {{ old('discount_type', $coupon->discount_type) == 'fixed' ? 'selected' : '' }}>Giảm cố định (VNĐ)</option>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="discount" id="discount" value="{{ old('discount', $coupon->discount) }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="scope" id="scope" required onchange="toggleProductSelect()">
                <option value="order" {{ old('scope', $coupon->scope) == 'order' ? 'selected' : '' }}>Toàn đơn hàng</option>
                <option value="product" {{ old('scope', $coupon->scope) == 'product' ? 'selected' : '' }}>Sản phẩm cụ thể</option>
            </select>
        </div>
        ...
        <div class="form-group" id="product_select_box" style="display: none;">
            <div class="form-group">
                ...
                <select name="product_id" id="product_id" class="select2" style="width: 100%;">
                    <option value="">-- Áp dụng cho toàn bộ sản phẩm --</option>
...
<style>
    ...
    .select2-container .select2-dropdown { border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.08); overflow: hidden; }
